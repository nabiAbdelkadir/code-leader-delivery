<template>
  ...
  <div class="topbar">
    ...
    <b-dropdown
        ...
        no-flip
    >
      <template v-slot:button-content class="position-relative">
        <span class="badge position-absolute bg-danger text-white " v-if="notificationsCount > 0">
          {{ notificationsCount }}
        </span>
        <div @click="$store.commit('setNotificationsCount' , 0)"
             class="btn btn-icon btn-clean btn-dropdown btn-lg mr-1 pulse pulse-primary"
        >

          <span class="svg-icon svg-icon-xl svg-icon-primary">
            <i class="fas fa-bell text-primary"></i>
            <inline-svg v-if="false" src="media/svg/icons/Code/Compiling.svg"/>
          </span>
    ...
    <b-dropdown
        ...
        no-flip
    >
      <template v-slot:button-content v-if="false">
    ...
    <KTQuickPanel v-if="false"></KTQuickPanel>
